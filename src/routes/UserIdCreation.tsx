...
//     setLoading(true)
//     axios
//       .get(`${process.env.NEXT_PUBLIC_API_URL}/api/V2.0/User/MES_UserDetails?UserId=${updateId}`, {
//         headers: {
...
//     id 
//       ? axios
//           .put(`${process.env.NEXT_PUBLIC_API_URL}/MES_UserDetails`, {
//             UserDetails: [
...
//           })
//       : axios
//           .post(`${process.env.NEXT_PUBLIC_API_URL}/MES_UserDetails`, {
//             UserDetails: [
//               Object.assign({}, answerValues, {
...
const UserIdCreation: React.FunctionComponent<IUserIdCreationProps> = ({ user }) => {
  const navigate = useNavigate()
  const query = useQuery()
  const id = query.get("id")
  const token = nookies.get(null).accessToken || ""
    ...
    axios
      ...
      .then((res) => {
        if (res.data && res.data.data) {
          setUpdateDataById(res.data.data) // ✅ new API structure
          console.log(res.data.data)
        }
      })
      .catch((error) => console.log(error))
  ...
  const handleSubmitForm = (answerValues: FormikValues, actions: FormikValues) => {
    ...
    id
      ? axios
          .put(
            `${process.env.NEXT_PUBLIC_API_URL}/User/MES_UserDetails`,
            {
              userDetails: [
                Object.assign({}, answerValues, {
                  modifiedBy: user?.email, // ✅ camelCase
                }),
              ],
            },
            { headers },
      ...
      : axios
          .post(
            `${process.env.NEXT_PUBLIC_API_URL}/User/MES_UserDetails`,
            {
              userDetails: [
                Object.assign({}, answerValues, {
                  createdBy: user?.email, // ✅ camelCase
                }),
              ],
            },
            { headers },
