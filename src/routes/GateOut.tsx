import { GATE_OUT_HEADER_DATA } from "@/utils/data"
...
interface IGateOutProps {}

const GateOut: React.FunctionComponent<IGateOutProps> = ({}) => {
  ...
  const handleClickEditAction = (infoSelectedRow: Record<string, any>) => {
    navigate(`/gate-out/configuration?id=${infoSelectedRow.OutEntryId}`)
  }
  ...
  const fetchAPI = () => {
    setLoading(true)
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/MES_Gate_Out`, {
        params: {
          page: currentPage,
          limit: itemsPerPage,
        },
  ...
  const renderDeleteModal = () => (
    <ModalDeleteComponent
      ...
      itemId={selectedViewUser?.OutEntryId}
      itemName={selectedViewUser?.OutEntryNumber}
      apiNameUrl="MES_Gate_Out"
    />
  )
  ...
  const renderModal = () => (
    <ModalComponent
      ...
      title={`Gate Out Details (${selectedViewUser?.OutEntryId})`}
    >
      {renderSelectedItems()}
    </ModalComponent>
  )
        ...
        <div className="w-full p-4 bg-neutral screen-height-media">
          <div className="w-full mb-2 text-right flex justify-between">
            {/* <span className="self-center font-bold text-sm">
              Gate Entry OUT (Overview)
            </span> */}
            <button
              className="btn btn-primary btn-outline btn-sm"
              onClick={() => navigate("/gate-out/configuration")}
            >
              Add New
            </button>
          </div>
          {loading ? (
            ...
            listDataFilter && (
              <>
                <BasicTable
                    tableHeader={GATE_OUT_HEADER_DATA}
