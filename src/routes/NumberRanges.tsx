import { NUMBER_RANGES_HEADER_DATA } from "@/utils/data"
...
interface INumberRangesProps {}

const NumberRanges: React.FunctionComponent<INumberRangesProps> = ({}) => {
  ...
  const fetchAPI = () => {
    setLoading(true)
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/MESNumberRanges`, {
        headers: {
  ...
  const renderDeleteModal = () => (
    <ModalDeleteComponent
      ...
      itemId={selectedViewUser?.rangeId}
      apiNameUrl="MESNumberRanges"
    />
  )
  ...
  const renderModal = () => (
    <ModalComponent
      ...
      title={`Number Ranges Details (${selectedViewUser?.rangeId})`}
    >
      {renderSelectedItems()}
    </ModalComponent>
  )
        ...
        <table className="table table-xs border border-base-300">
          <tbody>
            {Object.keys(selectedViewUser ?? {})
              .filter(
                (keyItem) => !["isDeleted", "assignedTo"].includes(keyItem),
              )
              .map((keyItem, index) => {
        ...
        <div className="w-full p-4 bg-neutral screen-height-media">
          <div className="w-full mb-2 text-right">
            <button
              ...
              onClick={() => navigate("/number-ranges/configuration")}
            >
              Add New
            </button>
          </div>
          {loading ? (
            ...
            currentItems && (
              <>
                <BasicTable
                  tableHeader={NUMBER_RANGES_HEADER_DATA}
