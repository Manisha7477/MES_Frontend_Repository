import {
  JOB_ALLOCATION_HEADER_DATA,
  JOB_ALLOCATION_FILTER_FORM_DATA,
} from "@/utils/data"
...
interface IJobAllocationProps {}

const JobAllocation: React.FunctionComponent<IJobAllocationProps> = ({}) => {
  ...
  const fetchAPI = () => {
    setLoading(true)
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/Job_Allocation`, {
        headers: {
  ...
  const renderModal = () => (
    <ModalComponent
      ...
      title={`Job Allocation Details (${selectedViewUser?.allocationId})`}
    >
      {renderSelectedItems()}
    </ModalComponent>
  )
      ...
      <div className="border rounded border-base-300">
        ...
        <div className="w-full p-4 bg-neutral screen-height-media">
                ...
                <div className="flex sm:flex-col lg:flex-row gap-4 w-full mb-2">
                  <div>
                    <LevelTopCreationFormField
                      formVariables={JOB_ALLOCATION_FILTER_FORM_DATA}
                    />
                  </div>
          ...
          {loading ? (
            ...
            <>
              <BasicTable
                tableHeader={JOB_ALLOCATION_HEADER_DATA}
