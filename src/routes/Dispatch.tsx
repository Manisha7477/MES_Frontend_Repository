import { DISPATCH_HEADER_DATA } from "@/utils/data"
...
const Dispatch: React.FunctionComponent<IDispatchProps> = ({}) => {
  ...
  const handleClickEditAction = (infoSelectedRow: Record<string, any>) => {
    navigate(`/dispatch/configuration?id=${infoSelectedRow.DispatchId}`)
  }
  ...
  const fetchAPI = () => {
    setLoading(true)
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/MES_Dispatching`, {
        params: {
          page: currentPage,
          limit: itemsPerPage,
        },
  ...
  const renderDeleteModal = () => (
    <ModalDeleteComponent
      ...
      itemId={selectedViewUser?.DispatchId}
      apiNameUrl="MES_Dispatching"
    />
  )
  ...
  const renderModal = () => (
    <ModalComponent
      ...
      title={`Dispatch Details (${selectedViewUser?.DispatchId})`}
    >
      {renderSelectedItems()}
    </ModalComponent>
  )
        ...
        <table className="table table-xs border border-base-300">
          <tbody>
            {Object.keys(selectedViewUser ?? {})
              .filter((keyItem) => !["IsDeleted"].includes(keyItem))
              .map((keyItem, index) => {
                return (
  ...
  return (
    <div className="w-full">
      <div className="border rounded border-base-300">
        ...
        <div className="w-full p-4 bg-neutral screen-height-media mt-10">
          <div className="w-full mb-2 text-right flex justify-between">
            ...
            <button
              ...
              onClick={() => navigate("/dispatch/configuration")}
            >
              Add New
            </button>
          </div>
          {loading ? (
            ...
            listDataFilter && (
              <>
                <BasicTable
                    tableHeader={DISPATCH_HEADER_DATA}
