import { GATE_IN_HEADER_DATA } from "@/utils/data"
...
interface IGateInProps {}

const GateIn: React.FunctionComponent<IGateInProps> = ({}) => {
  ...
  const handleClickEditAction = (infoSelectedRow: Record<string, any>) => {
    navigate(`/gate-in/configuration?id=${infoSelectedRow.InEntryId}`)
  }
  ...
  const fetchAPI = () => {
    setLoading(true)
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/MES_Gate_In`, {
        params: {
          page: currentPage,
          limit: itemsPerPage,
        },
  ...
  const renderDeleteModal = () => (
    <ModalDeleteComponent
      ...
      itemId={selectedViewUser?.InEntryId}
      itemName={selectedViewUser?.InEntryNumber}
      apiNameUrl="MES_Gate_In"
    />
  )
  ...
  const renderModal = () => (
    <ModalComponent
      ...
      title={`Gate In Details (${selectedViewUser?.InEntryId})`}
    >
      {renderSelectedItems()}
    </ModalComponent>
  )
        ...
        <div className="w-full p-4 bg-neutral screen-height-media">
          <div className="w-full mb-2 text-right flex justify-between">
            ...
            <button
              ...
              onClick={() => navigate("/gate-in/configuration")}
            >
              Add New
            </button>
          </div>
          {loading ? (
            ...
            listDataFilter && (
              <>
                <BasicTable
                    tableHeader={GATE_IN_HEADER_DATA}
