import { STORAGE_LOCATION_HEADER_DATA } from "@/utils/data"
...
interface IManageStorageLocationProps {}

const ManageStorageLocation: React.FunctionComponent<
  IManageStorageLocationProps
> = ({}) => {
  ...
  const handleClickEditAction = (infoSelectedRow: Record<string, any>) => {
    navigate(
      `/manage-storage-location/configuration?id=${infoSelectedRow.storageLocationId}`,
    )
  }
  ...
  const fetchAPI = useCallback(() => {
    setLoading(true)
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/MesStorage`, {
        headers: {
  ...
  const renderDeleteModal = () => (
    <ModalDeleteComponent
      ...
      itemId={selectedViewUser?.storageLocationId}
      itemName={selectedViewUser?.locationName}
      apiNameUrl="MesStorage"
    />
  )
  ...
  const renderModal = () => (
    <ModalComponent
      ...
      title={`Storage Location Details (${selectedViewUser?.storageLocationId})`}
    >
      {renderSelectedItems()}
    </ModalComponent>
  )
        ...
        <div className="w-full p-4 bg-neutral screen-height-media">
          <div className="w-full mb-2 text-right flex justify-end">
            ...
            <button
              ...
              onClick={() => navigate("/manage-storage-location/configuration")}
            >
              Add New
            </button>
          </div>
          {loading ? (
            ...
            currentItems && (
              <>
                <BasicTable
                  tableHeader={STORAGE_LOCATION_HEADER_DATA}
