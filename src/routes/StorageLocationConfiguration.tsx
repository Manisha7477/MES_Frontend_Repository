...
import { STORAGE_LOCATION_FORM_DATA, STORAGE_LOCATION_DATA } from "@/utils/data"
...
interface IStorageLocationConfigurationProps {
  user: IUser | null;
}

const StorageLocationConfiguration: React.FunctionComponent<
  IStorageLocationConfigurationProps
> = ({ user }) => {
  ...
  const fetchAPI = (updateId: string) => {
    setLoading(true)
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/MesStorage/${updateId}`,{
        headers: {
  ...
  const handleCancelForm = () => {
    navigate("/manage-storage-location")
  }
  const handleSubmitForm = (
    ...
    id
      ? axios
          .put(`${process.env.NEXT_PUBLIC_API_URL}/MesStorage`, {
            mesStorage: [
          ...
          .catch((error) => {
            toast.error("Error updating storage: " + error.message)
      ...
      : axios
          .post(`${process.env.NEXT_PUBLIC_API_URL}/MesStorage`, {
            mesStorage: [
              Object.assign({}, answerValues, {
          ...
          .then((_res) => {
            toast.success("New storage created successfully!")
            navigate("/manage-storage-location")
          })
          .catch((error) => {
            toast.error("Error creating new storage: " + error.message)
          })
          .finally(() => {
            setLoading(false)
            actions.setSubmitting(false)
    ...
    <div className="w-full px-2">
      <div className="border rounded border-base-300">
        <div className="bg-info rounded-t border-b border-base-300 font-bold px-4 py-1 mt-16">
          {id ? `Storage Location Update (${id})` : `Add New Storage Location`}
        </div>
        <div className="p-4 bg-neutral screen-height-media">
          {loading && <Loading />}
          <HorizontalLabelForm
            formVariables={STORAGE_LOCATION_FORM_DATA}
...
export default StorageLocationConfiguration
