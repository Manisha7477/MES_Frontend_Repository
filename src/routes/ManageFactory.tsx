import { FACTORY_HEADER_DATA } from "@/utils/data"
...
interface IManageFactoryProps {}

const ManageFactory: React.FunctionComponent<IManageFactoryProps> = ({}) => {
  ...
  const handleClickEditAction = (infoSelectedRow: Record<string, any>) => {
    navigate(`/manage-factory/configuration?id=${infoSelectedRow.factoryId}`)
  }
  ...
  const fetchAPI = useCallback(() => {
    setLoading(true)
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/MesFactory`, {
        headers: {
  ...
  const renderDeleteModal = () => (
    <ModalDeleteComponent
      ...
      itemId={selectedViewUser?.factoryId}
      itemName={selectedViewUser?.factoryName}
      apiNameUrl="MesFactory"
    />
  )
  ...
  const renderModal = () => (
    <ModalComponent
      ...
      title={`Factory Details (${selectedViewUser?.factoryId})`}
    >
      {renderSelectedItems()}
    </ModalComponent>
  )
    ...
    <div className="w-full">
      <div className="border rounded border-base-300">
        ...
        <div className="w-full p-4 bg-neutral screen-height-media">
          <div className="w-full mb-2 text-right flex justify-end mt-5">
            ...
            <button
              ...
              onClick={() => navigate("/manage-factory/configuration")}
            >
              Add New
            </button>
          </div>
          {loading ? (
            ...
            currentItems && (
              <>
                {/* <BasicTable
                  tableHeader={FACTORY_HEADER_DATA}
