...
import { DEPARTMENT_FORM_DATA } from "@/utils/data"
...
interface IDepartmentConfigurationProps {
  user: IUser | null
}

const DepartmentConfiguration: React.FunctionComponent<
  IDepartmentConfigurationProps
> = ({ user }) => {
  ...
  const initialDefaultValueData = initialFormikValues(DEPARTMENT_FORM_DATA)
  const formValidationSchemaData = formValidationSchema(DEPARTMENT_FORM_DATA)

  const fetchAPI = (updateId: string) => {
    setLoading(true)
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/MES_Department/${updateId}`, {
        headers: {
  ...
  const handleSubmitForm = (
    ...
    id
      ? axios
          .put(
            `${process.env.NEXT_PUBLIC_API_URL}/MES_Department`,
            {
              mesDepartment: [
      ...
      : axios
          ...
          .catch((error) => {
            toast.error("Error creating new department: " + error.message)
          })
          .finally(() => {
            setLoading(false)
            actions.setSubmitting(false)
    ...
    <div className="w-full px-2">
      <div className="border rounded border-base-300">
        ...
        <div className="p-4 bg-neutral screen-height-media">
          {loading && <Loading />}
          <HorizontalLabelForm
            formVariables={DEPARTMENT_FORM_DATA}
