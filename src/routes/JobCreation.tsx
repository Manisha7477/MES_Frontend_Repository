import {
  JOB_CREATION_HEADER_DATA,
  JOB_CREATION_FILTER_FORM_DATA,
} from "@/utils/data"
...
import Pagination from "@/components/Pagination" // Import the Pagination
import nookies from "nookies"

interface IJobCreationProps {}

const JobCreation: React.FunctionComponent<IJobCreationProps> = ({}) => {
  ...
  const handleClickEditAction = (infoSelectedRow: Record<string, any>) => {
    navigate(`/job-creation/configuration?id=${infoSelectedRow.jobId}`)
  }
  ...
  const fetchAPI = () => {
    setLoading(true)
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/JobCreation`, {
        headers: {
  ...
  const renderDeleteModal = () => (
    <ModalDeleteComponent
      ...
      itemId={selectedViewUser?.jobId}
      itemName={selectedViewUser?.jobName}
      apiNameUrl="JobCreation"
    />
  )
  ...
  const renderModal = () => (
    <ModalComponent
      ...
      title={`Job Creation Details (${selectedViewUser?.jobId})`}
    >
      {renderSelectedItems()}
    </ModalComponent>
  )

  const listDataFilter = listData.filter(
    //@ts-ignore
    (listDataItem) => !listDataItem.isDeleted,
  )
        ...
        <table className="table table-xs border border-base-300">
          <tbody>
            {Object.keys(selectedViewUser ?? {})
              .filter((keyItem) => !["isDeleted", "jobId"].includes(keyItem))
              .map((keyItem, index) => {
        ...
        <div className="w-full p-4 bg-neutral screen-height-media">
          <div className="w-full mb-2 text-right">
            <button
              ...
              onClick={() => navigate("/job-creation/configuration")}
            >
              CREATE NEW
            </button>
          </div>
                ...
                <div className="w-full flex items-center mb-2">
                  <div>
                    <LevelTopCreationFormField
                      formVariables={JOB_CREATION_FILTER_FORM_DATA}
                    />
                  </div>
                  <div className="w-24 sm:text-right self-end pb-3">
                    <button
                      type="submit"
          ...
          {loading ? (
            ...
            <>
              <BasicTable
                tableHeader={JOB_CREATION_HEADER_DATA}
