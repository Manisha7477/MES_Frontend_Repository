...
import { PICKING_PACKING_FORM_DATA } from "@/utils/data"
...
interface IPickingAndPackingConfigurationProps {
  user: IUser
}

const PickingAndPackingConfiguration: React.FunctionComponent<
  IPickingAndPackingConfigurationProps
> = ({ user }) => {
  ...
  const initialDefaultValueData = initialFormikValues(PICKING_PACKING_FORM_DATA)
  const formValidationSchemaData = formValidationSchema(
    PICKING_PACKING_FORM_DATA,
  )

  const fetchAPI = (updateId: string) => {
    setLoading(true)
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/MES_Picking_Packing/${updateId}`)
      .then((res) => {
        if (res.data.data) {
    ...
    updateData !== undefined
      ? updateData.ValidFrom && updateData.ValidTo
        ? {
            ...
            PackingDate: formatDateOnly(updateData.PackingDate),
            PickingDate: formatDateOnly(updateData.PickingDate),
          }
        : {}
      : {}
  ...
  const handleSubmitForm = (
    ...
    id
      ? axios
          .put(`${process.env.NEXT_PUBLIC_API_URL}/MES_Picking_Packing`, {
            MesPickingPacking: [
      ...
      : axios
          .post(`${process.env.NEXT_PUBLIC_API_URL}/MES_Picking_Packing`, {
            MesPickingPacking: [
              Object.assign({}, answerValues, {
    ...
    <div className="w-full px-2">
      <div className="border rounded border-base-300">
        <div className="bg-info rounded-t border-b border-base-300 font-bold px-4 py-1 mt-16">
          {id
            ? `Picking and Packing Operations Update (${id})`
            : `Add New Picking and Packing Operations`}
        </div>
        <div className="p-4 bg-neutral screen-height-media">
          {loading && <Loading />}
          <HorizontalLabelForm
            formVariables={PICKING_PACKING_FORM_DATA}
...
export default PickingAndPackingConfiguration
