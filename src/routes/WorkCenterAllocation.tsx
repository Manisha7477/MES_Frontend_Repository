import { WORK_CENTER_ALLOCATION_HEADER_DATA } from "@/utils/data"
...
interface IWorkCenterAllocationProps {}

const WorkCenterAllocation: React.FunctionComponent<
  IWorkCenterAllocationProps
> = ({}) => {
  ...
  const handleClickEditAction = (infoSelectedRow: Record<string, any>) => {
    navigate(
      `/work-center-allocation/configuration?id=${infoSelectedRow.AllocationId}`,
    )
  }
  ...
  const fetchAPI = () => {
    setLoading(true)
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/MES_WorkCenter_Allocation`, {
        headers: {
  ...
  const renderDeleteModal = () => (
    <ModalDeleteComponent
      ...
      apiNameUrl="MES_WorkCenter_Allocation"
    />
  )
  ...
  const renderModal = () => (
    <ModalComponent
      ...
      title={`WorkCenterAllocation Details (${selectedViewUser?.AllocationId})`}
    >
      {renderSelectedItems()}
    </ModalComponent>
  )
      ...
      <div className="border rounded border-base-300">
        ...
        <div className="w-full p-4 bg-neutral screen-height-media">
          ...
          {loading ? (
            ...
            <>
              <BasicTable
                  tableHeader={WORK_CENTER_ALLOCATION_HEADER_DATA}
...
export default WorkCenterAllocation
