...
import { JOB_ASSIGNMENT_FORM_DATA } from "@/utils/data"
...
interface IJobAssignmentConfigurationProps {
  user: IUser | null;
}

const JobAssignmentConfiguration: React.FunctionComponent<
  IJobAssignmentConfigurationProps
> = ({ user }) => {
  ...
  const initialDefaultValueData = initialFormikValues(JOB_ASSIGNMENT_FORM_DATA)
  const formValidationSchemaData = formValidationSchema(
    JOB_ASSIGNMENT_FORM_DATA,
  )

  const fetchAPI = (updateId: string) => {
    setLoading(true)
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/Job_Assignment/${updateId}`,{
        headers: {
  ...
  const handleSubmitForm = (
    ...
    id
      ? axios
          .put(`${process.env.NEXT_PUBLIC_API_URL}/Job_Assignment`, {
            jobAssignment: [
          ...
          .catch((error) => {
            toast.error("Error updating job assignment: " + error.message)
          })
          .finally(() => {
            setLoading(false)
      ...
      : axios
          .post(`${process.env.NEXT_PUBLIC_API_URL}/Job_Assignment`, {
            jobAssignment: [
              Object.assign({}, answerValues, {
          ...
          .then((_res) => {
            toast.success("New job assignment created successfully!")
            navigate("/job-assignment")
          })
          .catch((error) =>{
            toast.error("Error creating new job assignment: " + error.message)
          })
          .finally(() => {
            setLoading(false)
            actions.setSubmitting(false)
    ...
    <div className="w-full px-2">
      <div className="border rounded border-base-300">
        ...
        <div className="p-4 bg-neutral screen-height-media">
          {loading && <Loading />}
          <HorizontalLabelForm
            formVariables={JOB_ASSIGNMENT_FORM_DATA}
...
export default JobAssignmentConfiguration
