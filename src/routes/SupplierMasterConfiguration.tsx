...
import { SUPPLIER_MASTER_FORM_DATA } from "@/utils/data"
...
interface ISupplierMasterConfigurationProps {
  user: IUser
}

const SupplierMasterConfiguration: React.FunctionComponent<
  ISupplierMasterConfigurationProps
> = ({ user }) => {
  ...
  const initialDefaultValueData = initialFormikValues(SUPPLIER_MASTER_FORM_DATA)
  const formValidationSchemaData = formValidationSchema(
    SUPPLIER_MASTER_FORM_DATA,
  )

  const fetchAPI = (updateId: string) => {
    setLoading(true)
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/MES_Supplier_Master/${updateId}`)
      .then((res) => {
        if (res.data.data) {
  ...
  const handleSubmitForm = (
   ...
   // console.log(user)
    ...
    id
      ? axios
          .put(`${process.env.NEXT_PUBLIC_API_URL}/MES_Supplier_Master`, {
            MesSupplierMaster: [
          ...
          .catch((error) => {
            toast.error("Error updating supplier master: " + error.message)
      ...
      : axios
          .post(`${process.env.NEXT_PUBLIC_API_URL}/MES_Supplier_Master`, {
            MesSupplierMaster: [
              Object.assign({}, answerValues, {
          ...
          .then((_res) => {
            toast.success("New supplier master created successfully!")
            navigate("/supplier-master")
          })
          .catch((error) => {
            toast.error("Error creating new supplier master: " + error.message)
          })
          .finally(() => {
            setLoading(false)
            actions.setSubmitting(false)
    ...
    <div className="w-full px-2">
      <div className="border rounded border-base-300">
        <div className="bg-info rounded-t border-b border-base-300 font-bold px-4 py-1 mt-16">
          {id ? `Supplier Master Update (${id})` : `Add New Supplier master`}
        </div>
        <div className="p-4 bg-neutral screen-height-media">
          {loading && <Loading />}
          <HorizontalLabelForm
            formVariables={SUPPLIER_MASTER_FORM_DATA}
...
export default SupplierMasterConfiguration
