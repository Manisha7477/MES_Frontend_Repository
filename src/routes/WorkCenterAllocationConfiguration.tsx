...
import { WORK_CENTER_ALLOCATION_FORM_DATA } from "@/utils/data"
...
interface IWorkCenterAllocationConfigurationProps {
  user: IUser | null
}

const WorkCenterAllocationConfiguration: React.FunctionComponent<
  IWorkCenterAllocationConfigurationProps
> = ({ user }) => {
  ...
  const initialDefaultValueData = initialFormikValues(
    WORK_CENTER_ALLOCATION_FORM_DATA,
  )
  ...
  const fetchAPI = (updateId: string) => {
    ...
    axios
      .get(
        `${process.env.NEXT_PUBLIC_API_URL}/MES_WorkCenter_Allocation/${updateId}`,
        {
          headers: {
  ...
  const handleCancelForm = () => {
    navigate("/work-center-allocation")
  }
  const handleSubmitForm = (
    ...
    id
      ? axios
          .put(
            `${process.env.NEXT_PUBLIC_API_URL}/MES_WorkCenter_Allocation`,
            {
              MesWorkCenterAllocation: [
      ...
      : axios
          ...
          .then((_res) => {
            toast.success("New work center allocated successfully!")
            navigate("/work-center-allocation")
          })
          .catch((error) => {
            toast.error("Error allocating work center: " + error.message)
          })
          .finally(() => {
            setLoading(false)
            actions.setSubmitting(false)
    ...
    <div className="w-full px-2">
      <div className="border rounded border-base-300">
        ...
        <div className="p-4 bg-neutral screen-height-media">
          {loading && <Loading />}
          <HorizontalLabelForm
            formVariables={WORK_CENTER_ALLOCATION_FORM_DATA}
...
export default WorkCenterAllocationConfiguration
