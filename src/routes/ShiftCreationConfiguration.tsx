...
import { SHIFT_CREATION_FORM_DATA } from "@/utils/data"
...
interface IShiftCreationConfigurationProps {
  user: IUser | null;
}

const ShiftCreationConfiguration: React.FunctionComponent<
  IShiftCreationConfigurationProps
> = ({ user }) => {
  ...
  const initialDefaultValueData = initialFormikValues(SHIFT_CREATION_FORM_DATA)
  const formValidationSchemaData = formValidationSchema(
    SHIFT_CREATION_FORM_DATA,
  )

  const fetchAPI = (updateId: string) => {
    setLoading(true)
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/ShiftCreation/${updateId}`,{
        headers: {
  ...
  const handleSubmitForm = async (
    ...
    id
      ? axios
          .put(`${process.env.NEXT_PUBLIC_API_URL}/ShiftCreation`, {
            shiftCreation: [
          ...
          // .catch((error) => console.log(error))
          .catch((error) => {
            toast.error("Error updating shift details: " + error.message)
          })
          .finally(() => {
            setLoading(false)
      ...
      : axios
          .post(`${process.env.NEXT_PUBLIC_API_URL}/ShiftCreation`, {
            shiftCreation: [
              Object.assign({}, answerValues, {
          ...
          // .catch((error) => console.log(error))
          .catch((error) => {
            toast.error("Error creating new shift: " + error.message)
          })
          .finally(() => {
            setLoading(false)
            actions.setSubmitting(false)
    ...
    <div className="w-full px-2">
      <div className="border rounded border-base-300">
        <div className="bg-info rounded-t border-b border-base-300 font-bold px-4 py-1 mt-16">
          {id ? `Shift Update (${id})` : `Add New Shift`}
        </div>
        <div className="p-4 bg-neutral screen-height-media">
          {loading && <Loading />}
          <HorizontalLabelForm
            formVariables={SHIFT_CREATION_FORM_DATA}
...
export default ShiftCreationConfiguration
