...
import { BOM_FORM_DATA } from "@/utils/data"
...
const BomConfiguration: React.FunctionComponent<IBomConfigurationProps> = ({
  user,
}) => {
  ...
  const initialDefaultValueData = initialFormikValues(BOM_FORM_DATA)
  const formValidationSchemaData = formValidationSchema(BOM_FORM_DATA)

  const fetchAPI = (updateId: string) => {
    setLoading(true)
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/MES_BOM/${updateId}`, {
        headers: {
  ...
  const handleSubmitForm = async (
    ...
    id
      ? axios
          .put(
            `${process.env.NEXT_PUBLIC_API_URL}/MES_BOM`,
            {
              mesBom: [
          ...
          .catch((error) => {
            toast.error("Error updating BOM: " + error.message)
      ...
      : axios
          ...
          .catch((error) => {
            toast.error("Error creating new BOM: " + error.message)
          })
          .finally(() => {
            setLoading(false)
            actions.setSubmitting(false)
    ...
    <div className="w-full px-2">
      <div className="border rounded border-base-300">
        <div className="bg-info rounded-t border-b border-base-300 font-bold px-4 py-1 mt-16">
          {id ? `BOM Update (${id})` : `Add BOM`}
        </div>
        <div className="p-4 bg-neutral screen-height-media">
          {loading && <Loading />}
          <HorizontalLabelForm
            formVariables={BOM_FORM_DATA}
