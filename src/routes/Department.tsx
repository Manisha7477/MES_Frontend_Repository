import { DEPARTMENT_HEADER_DATA } from "@/utils/data"
...
const Department: React.FunctionComponent<IDepartmentProps> = ({}) => {
  ...
  const handleClickEditAction = (infoSelectedRow: Record<string, any>) => {
    navigate(`/department/configuration?id=${infoSelectedRow.departmentId}`)
  }
  ...
  const fetchAPI = () => {
    setLoading(true)
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/MES_Department`, {
        headers: {
  ...
  const renderDeleteModal = () => (
    <ModalDeleteComponent
      ...
      itemId={selectedViewUser?.departmentId}
      itemName={selectedViewUser?.departmentName}
      apiNameUrl="MES_Department"
    />
  )
  ...
  const renderModal = () => (
    <ModalComponent
      ...
      title={`Department Details (${selectedViewUser?.departmentId})`}
    >
      {renderSelectedItems()}
    </ModalComponent>
  )
        ...
        <div className="w-full p-4 bg-neutral screen-height-media">
          <div className="w-full mb-2 text-right">
            <button
              ...
              onClick={() => navigate("/department/configuration")}
            >
              CREATE NEW
            </button>
          </div>
          {loading ? (
            ...
            <>
              <BasicTable
                tableHeader={DEPARTMENT_HEADER_DATA}
