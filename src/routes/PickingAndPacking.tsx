import { PICKING_PACKING_HEADER_DATA } from "@/utils/data";
...
interface IPickingAndPackingProps {}

const PickingAndPacking: React.FunctionComponent<IPickingAndPackingProps> = ({}) => {
  ...
  const handleClickEditAction = (infoSelectedRow: Record<string, any>) => {
    navigate(`/picking-and-packing/configuration?id=${infoSelectedRow.OperationId}`);
  };
  ...
  const fetchAPI = () => {
    setLoading(true);
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/MES_Picking_Packing`, {
        params: {
          page: currentPage,
          limit: itemsPerPage,
        },
      ...
      .then((res) => {
        if (res.data.data) {
          const filteredData = res.data.data.filter((item: any) => !item.IsDeleted);
  ...
  const renderDeleteModal = () => (
    <ModalDeleteComponent
      ...
      itemId={selectedViewUser?.OperationId}
      apiNameUrl="MES_Picking_Packing"
    />
  );
  ...
  const renderModal = () => (
    <ModalComponent
      ...
      title={`Picking and Packing Operations Details (${selectedViewUser?.OperationId})`}
    >
      {renderSelectedItems()}
    </ModalComponent>
  );
        ...
        <div className="w-full p-4 bg-neutral screen-height-media">
          <div className="w-full mb-2 text-right">
            <button
              ...
              onClick={() => navigate("/picking-and-packing/configuration")}
            >
              Add New
            </button>
          </div>
          {loading ? (
            ...
            currentItems && (
              <>
                <BasicTable
                    tableHeader={PICKING_PACKING_HEADER_DATA}
