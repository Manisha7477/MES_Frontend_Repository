...
interface IPlantConfigurationProps {
  user: IUser | null
}

const PlantConfiguration: React.FunctionComponent<IPlantConfigurationProps> = ({
  user,
}) => {
  ...
  const fetchAPI = (updateId: string) => {
    setLoading(true)
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/MesPlant/${updateId}`, {
        headers: {
  ...
  const handleSubmitForm = async (
    ...
    id
      ? axios
          .put(
            `${process.env.NEXT_PUBLIC_API_URL}/MesPlant`,
            {
              mESplants: [
    ...
    <div className="w-full px-2">
      <div className="border rounded border-base-300">
        <div className="bg-info rounded-t border-b border-base-300 font-bold px-4 py-1 mt-16">
          {id ? `Plant Update (${id})` : `Add New Plant`}
        </div>
