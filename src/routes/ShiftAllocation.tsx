import { SHIFT_ALLOCATION_HEADER } from "@/utils/data"
...
interface IShiftAllocationProps {}

const ShiftAllocation: React.FunctionComponent<
  IShiftAllocationProps
> = ({}) => {
  ...
  const handleClickEditAction = (infoSelectedRow: Record<string, any>) => {
    navigate(
      `/shift-allocation/configuration?id=${infoSelectedRow.allocationId}`,
    )
  }
  ...
  const fetchAPI = () => {
    setLoading(true)
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/Shift_Allocation`, {
        headers: {
  ...
  const renderDeleteModal = () => (
    <ModalDeleteComponent
      ...
      fieldName1={selectedViewUser?.shiftName}
      apiNameUrl="Shift_Allocation"
    />
  )
  ...
  const renderModal = () => (
    <ModalComponent
      ...
      title={`Shift Assignment Details (${selectedViewUser?.allocationId})`}
    >
      {renderSelectedItems()}
    </ModalComponent>
  )
        ...
        <div className="w-full p-4 bg-neutral screen-height-media">
          <div className="w-full mb-2 text-right">
            <button
              ...
              onClick={() => navigate("/shift-allocation/configuration")}
            >
              CREATE NEW
            </button>
          </div>
          {loading ? (
            ...
            <>
              <BasicTable
                tableHeader={SHIFT_ALLOCATION_HEADER}
