...
import { JOB_CREATION_FORM_DATA, JOB_CREATION_DATA } from "@/utils/data"
...
interface IJobCreationConfigurationProps {
  user: IUser | null;
}

const JobCreationConfiguration: React.FunctionComponent<
  IJobCreationConfigurationProps
> = ({ user }) => {
  ...
  const initialDefaultValueData = initialFormikValues(JOB_CREATION_FORM_DATA)
  const formValidationSchemaData = formValidationSchema(JOB_CREATION_FORM_DATA)
  ...
  const handleSubmitForm = (
    ...
    id
      ? axios
          .put(`${process.env.NEXT_PUBLIC_API_URL}/JobCreation`, {
            jobCreation: [
      ...
      : axios
          .post(`${process.env.NEXT_PUBLIC_API_URL}/JobCreation`, {
            jobCreation: [
              Object.assign({}, answerValues, {
    ...
    <div className="w-full px-2">
      <div className="border rounded border-base-300">
        ...
        <div className="p-4 bg-neutral screen-height-media">
          {loading && <Loading />}
          <HorizontalLabelForm
            formVariables={JOB_CREATION_FORM_DATA}
...
export default JobCreationConfiguration
