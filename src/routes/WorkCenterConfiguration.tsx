...
import { WORK_CENTER_FORM_DATA } from "@/utils/data"
...
interface IWorkCenterConfigurationProps {
  user: IUser | null
}

const WorkCenterConfiguration: React.FunctionComponent<
  IWorkCenterConfigurationProps
> = ({ user }) => {
  ...
  const initialDefaultValueData = initialFormikValues(WORK_CENTER_FORM_DATA)
  const formValidationSchemaData = formValidationSchema(WORK_CENTER_FORM_DATA)

  const fetchAPI = (updateId: string) => {
    setLoading(true)
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/MESWorkCenter/${updateId}`, {
        headers: {
  ...
  const handleSubmitForm = (
    ...
    id
      ? axios
          .put(
            `${process.env.NEXT_PUBLIC_API_URL}/MESWorkCenter`,
            {
              mesWorkCenter: [
          ...
          .catch((error) => {
            toast.error("Error updating work center: " + error.message)
          })
          .finally(() => {
            setLoading(false)
      ...
      : axios
          ...
          .then((_res) => {
            toast.success("New work center created successfully!")

            navigate("/manage-work-center")
          })
          .catch((error) => {
            toast.error("Error creating new work center: " + error.message)
          })
          .finally(() => {
            setLoading(false)
            actions.setSubmitting(false)
    ...
    <div className="w-full px-2">
      <div className="border rounded border-base-300">
        <div className="bg-info rounded-t border-b border-base-300 font-bold px-4 py-1 mt-16">
          {id ? `Work Center Update (${id})` : `Add New Work Center`}
        </div>
        <div className="p-4 bg-neutral screen-height-media">
          {loading && <Loading />}
          <HorizontalLabelForm
            formVariables={WORK_CENTER_FORM_DATA}
...
export default WorkCenterConfiguration
