import { SHIFT_CREATION_HEADER_DATA } from "@/utils/data"
...
interface IShiftCreationProps {}

const ShiftCreation: React.FunctionComponent<IShiftCreationProps> = ({}) => {
  ...
  const fetchAPI = () => {
    setLoading(true)
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/ShiftCreation`, {
        headers: {
  ...
  const renderDeleteModal = () => (
    <ModalDeleteComponent
      ...
      itemId={selectedViewUser?.shiftId}
      itemName={selectedViewUser?.shiftName}
      apiNameUrl="ShiftCreation"
    />
  )
  ...
  const renderModal = () => (
    <ModalComponent
      ...
      title={`Shift Details (${selectedViewUser?.shiftId})`}
    >
      {renderSelectedItems()}
    </ModalComponent>
  )
        ...
        <table className="table table-xs border border-base-300">
          <tbody>
            {Object.keys(selectedViewUser ?? {})
              .filter((keyItem) => !["isDeleted", "shiftId"].includes(keyItem))
              .map((keyItem, index) => {
        ...
        <div className="w-full p-4 bg-neutral screen-height-media">
          <div className="w-full mb-2 text-right flex justify-end">
            ...
            <button
              ...
              onClick={() => navigate("/shift-creation/configuration")}
            >
              Add New
            </button>
          </div>
          {loading ? (
            ...
            <>
              <BasicTable
                tableHeader={SHIFT_CREATION_HEADER_DATA}
