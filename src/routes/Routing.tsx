import { ROUTING_HEADER_DATA } from "@/utils/data"
...
const Routing: React.FunctionComponent<IRoutingProps> = ({}) => {
  ...
  const handleClickEditAction = (infoSelectedRow: Record<string, any>) => {
    navigate(`/routing/configuration?id=${infoSelectedRow.routingId}`)
  }
  ...
  const fetchAPI = useCallback(() => {
    setLoading(true)
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/MESRouting`,{
        headers: {
  ...
  const renderDeleteModal = () => (
    <ModalDeleteComponent
      ...
      itemId={selectedViewUser?.routingId}
      itemName={selectedViewUser?.routingName}
      apiNameUrl="MESRouting"
    />
  )
  ...
  const renderModal = () => (
    <ModalComponent
      ...
      title={`Routing Details (${selectedViewUser?.routingId})`}
    >
      {renderSelectedItems()}
    </ModalComponent>
  )
        ...
        <div className="w-full p-4 bg-neutral screen-height-media">
          <div className="w-full mb-2 text-right">
            <button
              ...
              onClick={() => navigate("/routing/configuration")}
            >
              CREATE NEW
            </button>
          </div>
          {loading ? (
            ...
            currentItems && (
              <>
                <BasicTable
                  tableHeader={ROUTING_HEADER_DATA}
