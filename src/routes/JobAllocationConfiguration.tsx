...
import { JOB_ALLOCATION_FORM_DATA } from "@/utils/data"
...
interface IJobAllocationConfigurationProps {
  user: IUser | null;
}

const JobAllocationConfiguration: React.FunctionComponent<
IJobAllocationConfigurationProps
> = ({ user }) => {
  ...
  const initialDefaultValueData = initialFormikValues(JOB_ALLOCATION_FORM_DATA)
  const formValidationSchemaData = formValidationSchema(JOB_ALLOCATION_FORM_DATA)

  const fetchAPI = (updateId: string) => {
    setLoading(true)
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/Job_Allocation/${updateId}`,{
        headers: {
  ...
  const handleSubmitForm = (
    ...
    id
      ? axios
          .put(`${process.env.NEXT_PUBLIC_API_URL}/Job_Allocation`, {
            jobAllocation: [
          ...
          .catch((error) => {
            toast.error("Error updating job allocation: " + error.message)
          })
          .finally(() => {
            setLoading(false)
      ...
      : axios
          .post(`${process.env.NEXT_PUBLIC_API_URL}/Job_Allocation`, {
            jobAllocation: [
              Object.assign({}, answerValues, {
          ...
          .then((_res) => {
            toast.success("New job allocation created successfully!")
            navigate("/job-allocation")
          })
          .catch((error) => {
            toast.error("Error creating new job allocation: " + error.message)
          })
          .finally(() => {
            setLoading(false)
            actions.setSubmitting(false)
    ...
    <div className="w-full px-2">
      <div className="border rounded border-base-300">
        ...
        <div className="p-4 bg-neutral screen-height-media">
          {loading && <Loading />}
          <HorizontalLabelForm
            formVariables={JOB_ALLOCATION_FORM_DATA}
...
export default JobAllocationConfiguration
