import { PRODUCTION_VERSION_HEADER_DATA } from "@/utils/data"
...
interface IProductionVersionProps {}

const ProductionVersion: React.FunctionComponent<
  IProductionVersionProps
> = ({}) => {
  ...
  const handleClickEditAction = (infoSelectedRow: Record<string, any>) => {
    navigate(
      `/production-version/configuration?id=${infoSelectedRow.versionId}`,
    )
  }
  ...
  const fetchAPI = () => {
    setLoading(true)
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/MES_ProductionVersion`, {
        headers: {
  ...
  const renderDeleteModal = () => (
    <ModalDeleteComponent
      ...
      itemId={selectedViewUser?.versionId}
      itemName={selectedViewUser?.versionName}
      apiNameUrl="MES_ProductionVersion"
    />
  )
  ...
  const renderModal = () => (
    <ModalComponent
      ...
      title={`Production Version Details (${selectedViewUser?.versionId})`}
    >
      {renderSelectedItems()}
    </ModalComponent>
  )
        ...
        <div className="w-full p-4 bg-neutral screen-height-media">
          <div className="w-full mb-2 text-right">
            <button
              ...
              onClick={() => navigate("/production-version/configuration")}
            >
              CREATE NEW
            </button>
          </div>
          {loading ? (
            ...
            <>
              <BasicTable
                tableHeader={PRODUCTION_VERSION_HEADER_DATA}
...
export default ProductionVersion
