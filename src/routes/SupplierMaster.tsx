import { SUPPLIER_MASTER_HEADER_DATA } from "@/utils/data"
...
interface ISupplierMasterProps {}

const SupplierMaster: React.FunctionComponent<ISupplierMasterProps> = ({}) => {
  ...
  const handleClickEditAction = (infoSelectedRow: Record<string, any>) => {
    navigate(`/supplier-master/configuration?id=${infoSelectedRow.SupplierId}`)
  }
  ...
  const fetchAPI = () => {
    setLoading(true)
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/MES_Supplier_Master`, {
        params: {
          page: currentPage,
          limit: itemsPerPage,
        },
  ...
  const renderDeleteModal = () => (
    <ModalDeleteComponent
      ...
      itemId={selectedViewUser?.SupplierId}
      itemName={selectedViewUser?.SupplierName}
      apiNameUrl="MES_Supplier_Master"
    />
  )
  ...
  const renderModal = () => (
    <ModalComponent
      ...
      title={`Supplier Master Details (${selectedViewUser?.SupplierId})`}
    >
      {renderSelectedItems()}
    </ModalComponent>
  )
        ...
        <table className="table table-xs border border-base-300">
          <tbody>
            {Object.keys(selectedViewUser ?? {})
              .filter(
                (keyItem) => !["IsDeleted", "SupplierId"].includes(keyItem),
              )
              .map((keyItem, index) => {
                return (
        ...
        <div className="w-full p-4 bg-neutral screen-height-media">
          <div className="w-full mb-2 text-right flex justify-between">
            ...
            <button
              ...
              onClick={() => navigate("/supplier-master/configuration")}
            >
              Add New
            </button>
          </div>
          {loading ? (
            ...
            <>
              <BasicTable
                  tableHeader={SUPPLIER_MASTER_HEADER_DATA}
