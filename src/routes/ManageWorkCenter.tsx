import { WORK_CENTER_HEADER_DATA } from "@/utils/data"
...
interface IManageWorkCenterProps {}

const ManageWorkCenter: React.FunctionComponent<
  IManageWorkCenterProps
> = ({}) => {
  ...
  const handleClickEditAction = (infoSelectedRow: Record<string, any>) => {
    navigate(
      `/manage-work-center/configuration?id=${infoSelectedRow.workcenterId}`,
    )
    fetchAPI()
  }
  ...
  const fetchAPI = useCallback(() => {
    console.log("workcenter api calling...")
    setLoading(true)
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/MESWorkCenter`, {
        headers: {
  ...
  useEffect(() => {
    console.log("Component mounted and fetchAPI is being called...")
    fetchAPI() // Trigger API when component mounts
  }, [fetchAPI])
  ...
  const renderDeleteModal = () => (
    <ModalDeleteComponent
      ...
      itemId={selectedViewUser?.workcenterId}
      itemName={selectedViewUser?.workcenterName}
      apiNameUrl="MESWorkCenter"
    />
  )
  ...
  const renderModal = () => (
    <ModalComponent
      ...
      title={`Work Center Details (${selectedViewUser?.workcenterId})`}
    >
      {renderSelectedItems()}
    </ModalComponent>
  )
        ...
        <div className="w-full p-4 bg-neutral screen-height-media">
          <div className="w-full mb-2 text-right">
            <button
              ...
              onClick={() => navigate("/manage-work-center/configuration")}
            >
              Add New
            </button>
          </div>
          {loading ? (
            ...
            currentItems && (
              <>
                <BasicTable
                  tableHeader={WORK_CENTER_HEADER_DATA}
...
export default ManageWorkCenter
