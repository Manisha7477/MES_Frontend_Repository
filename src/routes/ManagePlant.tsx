import { PLANT_HEADER_DATA } from "@/utils/data"
...
interface IManagePlantProps {}

const ManagePlant: React.FunctionComponent<IManagePlantProps> = ({}) => {
  ...
  const handleClickEditAction = (infoSelectedRow: Record<string, any>) => {
    navigate(`/manage-plant/configuration?id=${infoSelectedRow.plantId}`)
  }
  ...
  const fetchAPI = useCallback(() => {
    setLoading(true)
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/MesPlant`, {
        headers: {
  ...
  const renderModal = () => (
    <ModalComponent
      ...
      title={`Plant Details (${selectedViewUser?.plantId})`}
    >
      {renderSelectedItems()}
    </ModalComponent>
  )
        ...
        <div className="w-full p-4 bg-neutral screen-height-media">
          <div className="w-full mb-2 text-right flex justify-end">
            ...
            <button
              ...
              onClick={() => navigate("/manage-plant/configuration")}
            >
              Add New
            </button>
          </div>
