import { STOCK_MOVEMENT_HEADER_DATA } from "@/utils/data"
...
const StockMovement: React.FunctionComponent<IStockMovementProps> = ({}) => {
  ...
  const handleClickEditAction = (infoSelectedRow: Record<string, any>) => {
    navigate(`/stock-movement/configuration?id=${infoSelectedRow.MovementId}`)
  }
  ...
  const fetchAPI = () => {
    setLoading(true)
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/MESStockMovements`, {
        params: {
          page: currentPage,
          limit: itemsPerPage,
        },
  ...
  const renderDeleteModal = () => (
    <ModalDeleteComponent
      ...
      itemId={selectedViewUser?.MovementId}
      apiNameUrl="MESStockMovements"
    />
  )
  ...
  const renderModal = () => (
    <ModalComponent
      ...
      title={`Stock Movements Details (${selectedViewUser?.MovementId})`}
    >
      {renderSelectedItems()}
    </ModalComponent>
  )
        ...
        <div className="w-full p-4 bg-neutral screen-height-media">
          <div className="w-full mb-2 text-right">
            <button
              ...
              onClick={() => navigate("/stock-movement/configuration")}
            >
              Add New
            </button>
          </div>
          {loading ? (
            ...
            listDataFilter && (
              <>
                <BasicTable
                    tableHeader={STOCK_MOVEMENT_HEADER_DATA}
                    tableData={listDataFilter}
