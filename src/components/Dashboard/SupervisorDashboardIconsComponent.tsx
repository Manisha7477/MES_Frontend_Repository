...
interface ISupervisorDashboardIconsComponentProps {}
type ISupervisorDashboardIconsData = {
  label: string
  iconName: string
  link?: string
}

const SupervisorDashboardIconsComponent: React.FunctionComponent<
  ISupervisorDashboardIconsComponentProps
> = ({}) => {
  ...
  const handleClick = (clickItem: ISupervisorDashboardIconsData) => {
    if (clickItem.link) {
  ...
  const renderItems = (buttonData: ISupervisorDashboardIconsData) => {
  ...
  return (
    <>
      <div className=" w-full sm:flex  flex-wrap items-baseline gap-4  ">
        {SupervisorDashboardIconsData.map((iconData) => renderItems(iconData))}
      </div>
      {modal && renderModal()}
    </>
  )
...
export default SupervisorDashboardIconsComponent
